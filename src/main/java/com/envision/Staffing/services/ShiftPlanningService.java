...
public class ShiftPlanningService {
	...
	public HourlyDetail[] getShiftPlan(Payload pl) throws IOException {
		...
		// Fetch workload array for 168 hours in a week
		for (int ii = 0; ii < 7; ii++) {
			for (int jj = 0; jj < 24; jj++) {
				work.fixedworkloadArray[k] = wl[ii].fixedworkloadArray[jj] / 1.2;
				work.workloadArray[k] = wl[ii].fixedworkloadArray[jj] / 1.2;
				k++;
			}
		}
		...
		for (int i = 0; i < 7; i++) {
			...
			for (Shift s : dayToshiftsmapping.get(i)) {
				...
				sw.write(wl[i].day + " | " + "Physician" + " | " + s.start_time + " | " + s.end_time + " | "
						+ s.no_of_hours + "\n");
				if (s.no_of_hours == 12)
					noOfTwelve[i]++;
